    ...
    ! Prefilter measured wind turbine signals to separate the filtering from the actual control actions
        ...
        IF (CntrPar%Fl_Mode > 0) THEN
            ...
            IF (LocalVar%iStatus == 0) THEN
                LocalVar%NacIMU_FA_AccF = SecLPFilter(dummy1, LocalVar%DT, CntrPar%F_FlCornerFreq(1), CntrPar%F_FlCornerFreq(2), LocalVar%iStatus, .FALSE., objInst%instSecLPF) ! Fixed Damping
                LocalVar%FA_AccF = SecLPFilter(dummy1, LocalVar%DT, CntrPar%F_FlCornerFreq(1), CntrPar%F_FlCornerFreq(2), LocalVar%iStatus, .FALSE., objInst%instSecLPF) ! Fixed Damping
            ELSE
            ...
            ENDIF
            LocalVar%NacIMU_FA_AccF = HPFilter(LocalVar%NacIMU_FA_AccF, LocalVar%DT, dummy2, LocalVar%iStatus, .FALSE., objInst%instHPF) 
            LocalVar%FA_AccF = HPFilter(LocalVar%FA_AccF, LocalVar%DT, dummy2, LocalVar%iStatus, .FALSE., objInst%instHPF) 
        ...
        LocalVar%We_Vw_F = LPFilter(LocalVar%WE_Vw, LocalVar%DT, dummy3, LocalVar%iStatus,.FALSE.,objInst%instLPF) ! 30 second time constant


        ! Control commands (used by WSE, mostly)
        LocalVar%VS_LastGenTrqF = SecLPFilter(LocalVar%VS_LastGenTrq, LocalVar%DT, CntrPar%F_LPFCornerFreq, dummy4, LocalVar%iStatus, .FALSE., objInst%instSecLPF)
        LocalVar%PC_PitComTF    = SecLPFilter(LocalVar%PC_PitComT, LocalVar%DT, CntrPar%F_LPFCornerFreq*0.25, dummy4, LocalVar%iStatus, .FALSE., objInst%instSecLPF)
