...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       PC_State = 1, is gain scheduled PI controller 
        ...
        IF (CntrPar%FlagLAC == 1) THEN ! if LAC is used
            LocalVar%PC_PitComT = PIControllerFF(LocalVar%PC_SpdErr, LocalVar%PC_KP, LocalVar%PC_KI, LocalVar%FF_PitchRate,LocalVar%PC_MinPit, LocalVar%PC_MaxPit, LocalVar%DT, LocalVar%BlPitch(1), LocalVar%piP, LocalVar%restart, objInst%instPI)
        ELSE
