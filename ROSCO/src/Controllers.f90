...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       PC_State = 1, is gain scheduled PI controller 
        ...
        ELSE
            IF (CntrPar%FlagLAC == 1) THEN
            LocalVar%PC_PitComT = PIControllerFF(LocalVar%PC_SpdErr, LocalVar%PC_KP, LocalVar%PC_KI, LocalVar%FF_PitchRate,LocalVar%PC_MinPit, LocalVar%PC_MaxPit, LocalVar%DT, LocalVar%BlPitch(1), .FALSE., objInst%instPI)
            ELSE
...
!-------------------------------------------------------------------------------------------------------------------------------
    SUBROUTINE IPC(CntrPar, LocalVar, objInst)
        ...
        IF (CntrPar%Y_ControlMode == 2) THEN
            ...
            Y_MErrF_IPC = PIController(Y_MErrF, CntrPar%Y_IPC_KP(1), CntrPar%Y_IPC_KI(1), -CntrPar%Y_IPC_IntSat, CntrPar%Y_IPC_IntSat, LocalVar%DT, dummy1, .FALSE., objInst%instPI)
        ELSE
...
!-------------------------------------------------------------------------------------------------------------------------------
    SUBROUTINE ForeAftDamping(CntrPar, LocalVar, objInst)
        ...
        ! Body
        LocalVar%FA_AccHPFI = PIController(LocalVar%FA_AccHPF, dummy1, CntrPar%FA_KI, -CntrPar%FA_IntSat, CntrPar%FA_IntSat, LocalVar%DT, dummy1, .FALSE., objInst%instPI)
...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       Fl_Mode = 2, Proportional feedback of nacelle velocity (rotational)
        ...
        FA_vel = PIController(LocalVar%FA_AccF, dummy1, dummy5, -dummy6 , dummy6 ,LocalVar%DT, dummy1, .FALSE., objInst%instPI) ! NJA: should never reach saturation limits....
        NacIMU_FA_vel = PIController(LocalVar%NacIMU_FA_AccF, dummy1, dummy5, -dummy6 , dummy6 ,LocalVar%DT, dummy1, .FALSE., objInst%instPI) ! NJA: should never reach saturation limits....
        ...
        IF (CntrPar%Flp_Mode >= 1) THEN
            IF ((LocalVar%iStatus == 0) .AND. (CntrPar%Flp_Mode >= 1)) THEN
                ...
                IF (CntrPar%Flp_Mode == 2) THEN
                    LocalVar%Flp_Angle(K) = PIIController(RootMyb_VelErr(K), 0 - LocalVar%Flp_Angle(K), CntrPar%Flp_Kp, CntrPar%Flp_Ki, dummy7, -CntrPar%Flp_MaxPit , CntrPar%Flp_MaxPit , LocalVar%DT, dummy1, .TRUE., objInst%instPI)
                ENDIF
            ...
            ELSEIF (CntrPar%Flp_Mode == 2) THEN
                DO K = 1,LocalVar%NumBl
                    ...
                    LocalVar%Flp_Angle(K) = PIIController(RootMyb_VelErr(K), 0 - LocalVar%Flp_Angle(K), CntrPar%Flp_Kp, CntrPar%Flp_Ki, dummy7, -CntrPar%Flp_MaxPit , CntrPar%Flp_MaxPit , LocalVar%DT, dummy1, .FALSE., objInst%instPI)
                    ! Saturation Limits
