...
TextCell{end+1} = num2str(Turbulence.Field.n2_use);
TextCell{end+1} = num2str(Turbulence.Field.n3_use);
...
Text_temp = ['U' num2str(Turbulence.URef) '_' Text_temp];
...
if ~isfile([TargetDir '\' TextCell{end}])
    copyfile([ExeDir '\InputSample4DMann.inp'],[Text_temp '.inp'])
    fileID      = fopen([Text_temp '.inp'],'w');            
    fprintf(fileID,'%s\n',TextCell{:});
    fclose(fileID);
    dos([ExeDir '\MannTurb4D_v2.exe',' ',Text_temp '.inp']);
