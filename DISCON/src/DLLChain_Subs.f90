! Name:   The subroutines of the master (wrapper) DLL.
...
! -----------------------------------------------------------------------------------
    ...
    SUBROUTINE SetDLLParameters(avrSWAP, accINFILE, size_avcMSG, DLL_Type, DLL_ErrVar)
        ...
        IF (iStatus == 0) THEN ! .TRUE. if we're on the first call to the DLL
            ...
            ! Description:
            print *, '--------------------------------------------------------------------'
            print *, 'A DLL chain for public use in developing lidar data processing algorithms'
    ...
    SUBROUTINE ReadDLLParameterFileSub(DLL_Type,accINFILE, accINFILE_size,DLL_ErrVar)!, accINFILE_size)
        ...
        DO i=1, DLL_Type%NumberOfsubDLLs
           CALL ParseInput(UnControllerParameters,CurLine,'SubDLLName',accINFILE(1),DLL_TYPE%DLLFILENAME(i),DLL_ErrVar)
           CALL ParseInput(UnControllerParameters,CurLine,'SubDLLInputName',accINFILE(1),DLL_TYPE%DLLINPUTFILENAME(i),DLL_ErrVar)
           ppos = SCAN(TRIM(DLL_TYPE%DLLFILENAME(i)),".", BACK= .true.)
             DLL_TYPE%PROCNAME(i) = DLL_TYPE%DLLFILENAME(i)(1:ppos-1)
             
            IF (len_trim( DLL_TYPE%PROCNAME(i))==0) THEN
