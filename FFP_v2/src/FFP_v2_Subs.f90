...
CONTAINS
    ...
    SUBROUTINE ReadLidarParameterFileSub(LidarVar, accINFILE, accINFILE_size, ErrVar)
		...
		CALL ParseInput(UnControllerParameters,CurLine,'f_cutoff_LP',accINFILE(1),LidarVar%f_cutoff_LP,ErrVar)
        CALL ParseInput(UnControllerParameters,CurLine,'FlagNotch', accINFILE(1),LidarVar%FlagLPF, ErrVar)
		CALL ParseInput(UnControllerParameters,CurLine,'f_cutoff_Notch',accINFILE(1),LidarVar%f_cutoff_Notch,ErrVar)
    ...
    SUBROUTINE CalculateFeedForwardPitchRate(avrSWAP, LidarVar, ErrVar)
		...
		IF (LidarVar%FlagLPF == 1) THEN
			LPF_inst = 1
			LidarVar%REWS_f      	= LPFilter(LidarVar%REWS, LidarVar%DT, LidarVar%f_cutoff_LP, LidarVar%iStatus, .FALSE., LPF_inst)
		ELSE
		...
		IF (LidarVar%FlagNotch == 1) THEN
			Notch_inst = 1
			LidarVar%REWS_f      	= NotchFilter(LidarVar%REWS_f, LidarVar%DT, LidarVar%f_cutoff_Notch, LidarVar%betaNum_Notch, LidarVar%betaDen_Notch, LidarVar%iStatus, .FALSE., Notch_inst)
		ELSE
